...
// Sort functionality
var isAscending = true; // Track the current sort order

function sortBars() {
    // Sort the dataset based on the current order
    dataset.sort(isAscending ? d3.ascending : d3.descending);

    // Update the xScale domain based on the sorted order
    xScale.domain(d3.range(dataset.length));

    // Rebind the data with the sorted dataset and assign a key function
    svg1.selectAll("rect")
        .data(dataset, function(d) { return d; })  // Use data value as the key
        .transition()
        .duration(800) // Set transition duration
        .ease(easingFunction) // Use the selected easing function
        .attr("x", function(d, i) {
            return xScale(i); // Update x position based on new index
        });
...
d3.select("#sortBtn").on("click", sortBars);
